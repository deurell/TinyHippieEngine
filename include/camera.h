...
public:
  ...
  void translate(const glm::vec3 &v) { mPosition += v * mOrientation; }
	void translate(float x, float y, float z) { mPosition += glm::vec3(x, y, z) * mOrientation; }

	void rotate(float angle, const glm::vec3 &axis) { mOrientation *= glm::angleAxis(angle, axis * mOrientation); }
	void rotate(float angle, float x, float y, float z) { mOrientation *= glm::angleAxis(angle, glm::vec3(x, y, z) * mOrientation); }
