...
void Renderer::updateSource(DL::ShaderPlaneNode *node, float elapsed,
  ...
  setSourceParams(node, elapsed, pulse, load);
}
...
void Renderer::updateBlocker(DL::ShaderPlaneNode *node, float &glow,
  ...
  setBlockerParams(node, glow, storedEnergy, hitPoint);
}
...
void Renderer::updatePortal(DL::ShaderPlaneNode *entryNode,
  ...
  setPortalParams(entryNode, elapsed, phase, glow, false, entryHitPoint);
  setPortalParams(exitNode, elapsed, phase + 0.5f, glow, true, exitHitPoint);
}
...
void Renderer::updateFilter(DL::ShaderPlaneNode *node, float &passGlow,
  ...
  setFilterParams(node, passGlow, blockGlow, hitPoint);
}
...
void Renderer::updateSplitter(DL::ShaderPlaneNode *node, float &glow,
  ...
  setSplitterParams(node, glow, hitPoint);
}
...
void Renderer::updateTarget(DL::ShaderPlaneNode *node, bool alive,
  ...
  setTargetParams(node, elapsed, phase, hitFlash);
}
...
void Renderer::updateSelection(DL::ShaderPlaneNode *node,
  ...
  setSelectionParams(node, elapsed, flash);
}
...
void Renderer::updateExplosion(DL::ShaderPlaneNode *node, float amount,
  ...
  if (!active) {
    hideNode(node);
    setExplosionParams(node, 1.0f, 0.0f, seed, 1.55f);
    return;
  }
  setExplosionParams(node, amount, energy, seed, 1.55f);
}
